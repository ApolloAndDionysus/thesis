...
\caption{$4827$ arms. Difference of the recommended and optimal arm's mean as a function of total pulls. }
\label{fig:newyorker}
\vspace{-1.5em}
\end{wrapfigure}
%
...
The bold blue line is the best-arm identification algorithm lil'UCB \citep{Jamieson2014lilU} executed on all $4827$ arms, lil'UCB-$X$ for $X$ in $\{10,100,1000,10000\}$ is lil'UCB run on $X$ arms randomly drawn with replacement from the $4827$ arms, and Successive Halving is the proposed algorithm of this work. Each algorithm outputs the empirical best arm at any given total budget of pulls, breaking ties randomly.
...
We also observe that Successive Halving appears to naturally navigate this tradeoff.  
...
\noindent\textbf{Objective:} For a fixed reservoir distribution $\nu_0$ and $\epsilon,\delta \in (0,1)$, minimize the time $\tau \in \mathbb{N}$ such that $\P( \mu_{J_\tau} \leq \mu_* + \epsilon ) \geq 1-\delta$ where $\mu_* = \inf \{ x : x \in \text{support}(\nu_0) \}$.

\noindent Said another way, minimize \emph{simple regret} \citep{bubeck2009pure,DBLP:journals/corr/CarpentierV15}.
...
Quantile problems have also been studied in the pure-exploration setting, such as identifying an arm $\epsilon$-close to $\mu_q$ \citep{chaudhuri2017pac,aziz2018pure}.
