...
\label{alg-ST}
\KwData{ budget $n$, target toxicity $\theta$ }
\textbf{Initialization:} Set of dose levels $S_0 \leftarrow \{1, \dots, K\}$ \;
  \For{$r \leftarrow 0$ to $\lceil\log_2(K)\rceil-1$}{
    ...
    Based on their response compute $\hat p_k^r$, the empirical toxicity of dose $k$ based on these $t_r$ samples \;
    Compute $S_{r+1}$ the set of $\lceil|S_r|/2\rceil$ arms with 
